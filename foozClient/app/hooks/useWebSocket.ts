...
export const useWebSocket = (connectionEndpoint: string) => {
  ...
  const [onMessage, setOnMessage] = useState<(data: string, ws?: WebSocket) => void>((data, ws) => () => {console.log(data);});
  const [onConnect, setOnConnect] = useState<(event: Event, ws?: WebSocket) => void>((data, ws) => () => {console.log(data);});
  const [onDisconnect, setOnDisconnect] = useState<(event: Event, ws?: WebSocket) => void>((data, ws) => () => {console.log(data);});
  ...
  useEffect(function sendMessage() {
